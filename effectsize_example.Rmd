...
data %>% 
  ...
  ggplot(aes(x = group, y = completion_time_sec)) +
...
```{r}
t_result <- 
  t.test(completion_time_sec ~ group, data = data) %>%
  tidy() # put result in tabular format
t_result
```
...
t_result %>% 
  ...
  xlab("Difference in task completion time (s)") +
  ylab("")
```
...
```{r}
library(effsize)
cohen_d <- cohen.d(completion_time_sec ~ group, data = data)

# manual calculation
data_A <- (data %>% filter(group == "A"))[["completion_time_sec"]]
data_B <- (data %>% filter(group == "B"))[["completion_time_sec"]]
sd_A <- sd(data_A)
sd_B <- sd(data_B)
...
data_A <- (data %>% filter(group == "A"))[["completion_time_sec"]]
data_B <- (data %>% filter(group == "B"))[["completion_time_sec"]]
wilcox_result <- wilcox_test(completion_time_sec ~ factor(group), data = data)
