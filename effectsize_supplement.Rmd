...
log_bayes_priors <- c(
    ...
    prior(normal(-0.75, 0.825), class = b, coef = intercept, nlpar = sigma),
...
t_interval %>%
  ...
  bind_rows(log_interval, log_bayes_interval, log_bayes_informed_interval) %>%
  ggplot(aes(y = method, x = mean_difference, xmin = mean_difference.lower, xmax = mean_difference.upper)) + 
  geom_violinh(aes(y = method, x = mean_difference), data = bayes_samples, 
    inherit.aes = FALSE, color = NA, fill = "gray65") +
  geom_pointrangeh() +
