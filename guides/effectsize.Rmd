...
This is a controversial issue. More pointers [here](#effectsize_faq_controversial)
```

#### Standardized effect sizes
Standardized effect sizes are useful when effects obtained from different experiments and/or expressed in different units need to be combined or compared [@Cumming2014a], e.g., a metaanalysis of the literature. 
...
Cohen's rule of thumb for the small, mediuam, and large effect are specific to his domain and has been shown to not generalizable.
See [What about Cohen's small, medium, and large effect sizes?](#effectsize_faq_small_medium_large)
...
Reporting simple effect sizes also allow future researchers to estimate and interpret variance with Bayesian method.
Reporting the simple effect size also consistent with the pinciple of simplicity

▸ Exemplar: [simple effect size](#effectsize_exemplar_simple) and [standardized effect size](#effectsize_exemplar_standardized)

▸ For further discussion about the standardized vs. simple effect sizes, [see a blog post: Can we call mean differences "effect sizes"?](http://transparentstatistics.org/2018/07/05/meanings-effect-size/)
