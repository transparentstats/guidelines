...
coreCount <- ifelse(coreCount < 1, 1, coreCount)
...
system.time(
  bootResults <- parLapply(
    cl, # the cluster of cores
    rep(bootstrapIterationCount, coreCount), # how many runs for each core
    analyzeManyIterations) # the function to run in each core
)
...
anovaResults %>% 
  ...
  # plot and mapping
  ggplot(aes(x = effect, y = ges, ymin = ESPercentileLo, ymax = ESPercentileHi)) +
